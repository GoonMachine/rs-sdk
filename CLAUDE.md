...
6. Who-reads-what: `learnings/README.md` (open one file when stuck). `git pull origin main` before a long job. Your `cloud-agent-*.md` only. Do **not** read `learnings/operator.md`. A = Restless Ghost first on `qstboot1`. B = mule + rejoin, not cows. Do not share bot names.
