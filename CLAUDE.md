...
6. Read `learnings/strategy-compete-koth.md` and `learnings/server-diffs.md` before a long-horizon goal, then `learnings/owner-context.md`. Hiscores are total level / playtime (cap 1881). First Cloud job is Phase 1 probes, not a hill raid. Two-agent split: Agent A = tick/quest-XP/polygon then Waterfall (`learnings/cloud-agent-a.md`); Agent B = PvP death-mark + positions then rats/food (`learnings/cloud-agent-b.md`). Do not share bot names.
