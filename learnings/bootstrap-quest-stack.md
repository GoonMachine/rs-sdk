...
| 3. Waterfall | `quest_waterfall.rs2` `137500` att/str | +343,750 each | att 87 / str 83 / **~cb 65**. Prot melee in the dungeon. White Wolf = Taverley→Catherby **trail**, not south of z=3400. Items: rope (Ned Draynor `(3100, 3258)` / 15gp, `ned.rs2` — not a mugger grind), 6 air, 6 water, 6 earth. Raft miss stays `started` — talk to Hudon on the island (`hudon.rs2`); bookcase only after that ([`dialog.md`](dialog.md)). |
